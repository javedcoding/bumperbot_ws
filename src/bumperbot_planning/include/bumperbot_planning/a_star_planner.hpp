...
struct GraphNode
{
    int x;
    int y;
    int cost; //this is the cost between 2 nodes
    double heuristic; //probabilistic or heuristic distance between a node and dstination node
    ...
    {
        return (cost + heuristic) > (other.cost + other.heuristic); //total cose will be addition of heuristic and node to node cost value
    }
    bool operator==(const GraphNode & other) const
    {
...
private:
    ...
    double manhattanDistance(const GraphNode & node, const GraphNode & goal_node); //Calculates the manhattan distance between current node and destination node
