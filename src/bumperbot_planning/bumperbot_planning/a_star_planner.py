...
class GraphNode:
    def __init__(self, x, y, cost=0, heuristic=0, prev_node=None):
        self.x = x
        self.y = y
        self.cost = cost
        self.heuristic = heuristic #this heuristic is calculated using Manhattan distance instead of Eucleadean distance
        self.prev_node = prev_node

    def __lt__(self, other):
        return (self.cost + self.heuristic) < (other.cost + other.heuristic) #so all calculation will be based on cost + heuristic 
...
class AStarPlanner(Node):
    def __init__(self):
        ...
        self.map_pub = self.create_publisher(OccupancyGrid, "/a_star/visited_map", map_qos)
    ...
    def plan(self, start:Pose, goal:Pose):
        ...
        while not pending_nodes.empty() and rclpy.ok():
            ...
            self.visited_map_.data[self.pose_to_cell(active_node)] = -106 #this makes the visited path value on map as orange in rviz
    ...
    def manhattan_distance(self, node: GraphNode, goal_node: GraphNode):
        #( |Yb-Ya| + |Xb-Xa| )
        return abs(node.x - goal_node.x) + abs(node.y - goal_node.y) #This is the Manhattan distance calculation
